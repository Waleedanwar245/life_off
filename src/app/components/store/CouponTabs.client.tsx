// app/components/CouponTabs.client.tsx
"use client";
...
type Props = {
  data: any;
  preFetchedLatestStores?: any[];
  preFetchedSimilarCoupons?: any[];
  preFetchedSimilarStores?: any[];
};

export default function CouponTabs({
  data,
  preFetchedLatestStores,
  preFetchedSimilarCoupons,
  preFetchedSimilarStores,
}: Props) {
  ...
  // State for API data — initialize from server preFetched props where available
  const [latestStores, setLatestStores] = useState<any[]>(preFetchedLatestStores || []);
  const [similarCoupons, setSimilarCoupons] = useState<any[]>(preFetchedSimilarCoupons || []);
  const [similarStores, setSimilarStores] = useState<any[]>(preFetchedSimilarStores || []);
  ...
  // Fetch latest stores client-side only if server did not pre-fetch them
  useEffect(() => {
    if (preFetchedLatestStores && preFetchedLatestStores.length > 0) return;
  ...
  // Fetch similar coupons and stores based on category (client only if not pre-fetched)
  useEffect(() => {
    const categoryId = data?.store?.categoryIds?.[0];
    if (!categoryId) return;

    if (preFetchedSimilarCoupons && preFetchedSimilarCoupons.length > 0) {
      // already pre-fetched server-side
      return;
    }
    ...
    const fetchSimilarCoupons = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/categories/${categoryId}/coupons`);
        if (response.ok) {
          const json = await response.json();
          setSimilarCoupons(json);
        }
      } catch (error) {
    ...
    fetchSimilarCoupons();
    fetchSimilarStores();
  }, [data?.store?.categoryIds, preFetchedSimilarCoupons]);

  // --- rest of your original functions and JSX (kept intact) ---
  ...
  const activeCoupons = coupons.filter((coupon: any) => coupon.type === "Active").sort((a: any, b: any) => a.rank - b.rank);
  ...
  return (
    <div className="max-w-[1280px] font-sans mx-auto px-4 py-8">
      ...
      <div className="sticky top-0 z-10 bg-white border-b">
        <div className="flex">
          <button onClick={() => scrollToSection(couponsRef as any)} className="px-4 py-2 text-sm font-medium text-green-600 border-b-2 border-green-600">
            Coupons
          </button>
                      ...
                      filteredCoupons.map((coupon: any) => {
                        ...
                        return (
                          <div key={coupon.id} className="border rounded-md p-0 mb-4 px-4 ">
                                      ...
                                      {coupon.isExclusive && (
                                        <span>
                                          ...
                                          <Tag
                                            ...
                                            style={{ marginLeft: "10px", background: "#789A1A", color: "white", height: "22px", lineHeight: "18px" }}
                                            color="success"
                                          >
                                            Exclusive
                                          </Tag>
                            ...
                            <div className="text-xs text-gray-600 flex items-center mt-0 p-2 border-t cursor-pointer" onClick={() => setExpandedId(expandedId === coupon.id ? null : coupon.id)}>
            ...
            <div className="w-full lg:w-1/3">
              ...
              <div className="mb-6 hidden md:block">
                ...
                <div className="space-y-2">
                  <button className={`w-full transition-colors rounded-full py-2 font-medium text-sm ${activeFilter === "all" ? "bg-[#7FA842] text-white hover:bg-[#81ac41]" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} onClick={() => setActiveFilter("all")}>
                    All ({activeCouponCount})
                  </button>
                  <button className={`w-full transition-colors rounded-full py-2 font-medium text-sm ${activeFilter === "deals" ? "bg-[#7FA842] text-white hover:bg-[#81ac41]" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} onClick={() => setActiveFilter("deals")}>
                    Deals ({activeDealsCount})
                  </button>
                  <button className={`w-full transition-colors rounded-full py-2 font-medium text-sm ${activeFilter === "codes" ? "bg-[#7FA842] text-white hover:bg-[#81ac41]" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} onClick={() => setActiveFilter("codes")}>
              ...
              <div className="custom-class border rounded-lg p-5 mb-6 bg-white shadow-lg">
                {data?.store?.storeDescription ? (
                  <p className="text-sm text-gray-700 mb-4 leading-relaxed" dangerouslySetInnerHTML={{ __html: data?.store?.storeDescription }} />
                ) : (
        ...
        <Row>
          <Col xs={24} md={18}>
            {data?.store?.products?.length > 0 && (
              <div className="my-16">
                <p className="text-[25px] md:text-[35px] font-bold mb-4 text-gray-800">{data?.store?.name} Products</p>
          ...
          <Col xs={24} md={16} ref={storeInfoRef}>
            <div className="pt-8 border-t">
              <div className="custom-class prose lg:prose-xl">
                {data?.store?.storeArticle ? <div dangerouslySetInnerHTML={{ __html: data?.store?.storeArticle }} /> : <p className="text-gray-500">No article available at the moment. Stay tuned for more updates!</p>}
              </div>
            </div>
          </Col>
